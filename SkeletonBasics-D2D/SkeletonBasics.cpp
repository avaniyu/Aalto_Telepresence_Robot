...
//------------------------------------------------------------------------------
// Aalto Protopaja -- Futurice 8 team
//	   8.7.2017
//------------------------------------------------------------------------------
...
// declare 21 joints' coordinate(x, y, z) as global variables
static float joint[20][3] = { 0. };
...
void CSkeletonBasics::ProcessSkeleton()
{
    ...
    for (int i = 0 ; i < NUI_SKELETON_COUNT; ++i)
    {
        ...
        if (NUI_SKELETON_TRACKED == trackingState)
        {
			...
			FILE *f = fopen("joint_x.txt", "a");
			
			// export no.m joint coordinate to 2D array joint[][]
			// SkeletonData[6]: a collection of tracked people
			// SkeletonPositions[20]: a suite of joint information of one tracked person
			for (int m = 0; m < 20; m++) {
				joint[m][0] = skeletonFrame.SkeletonData[i].SkeletonPositions[m].x;
				joint[m][1] = skeletonFrame.SkeletonData[i].SkeletonPositions[m].y;
				joint[m][2] = skeletonFrame.SkeletonData[i].SkeletonPositions[m].z;

				// writing some sample data to .txt
				fprintf(f, "%.2f\n", joint[m][0]);
			}
			fprintf(f, "-------------\n");
			fclose(f);
...
{      
	...
	//FILE *f = fopen("head_coordinate.txt", "a");
